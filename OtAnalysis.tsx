...
export default function EmployeeReview() {
  ...
  const empData = Array.from(allEmps).map(emp=>{
    const otRows=otByEmp[emp]||[]; const bonusRows=bonusByEmp[emp]||[];
    const ot=sumNum(otRows,'otDollars'); const hrs=sumNum(otRows,'otHours'); const bonus=sumNum(bonusRows,'bonusDollars'); const combined=ot+bonus;
    const flag=combined>10000?'High Priority':combined>5000?'Moderate':'Normal';
    return{emp,fac:otRows[0]?.facility||bonusRows[0]?.facility||'—',pos:otRows[0]?.position||bonusRows[0]?.position||'—',dept:otRows[0]?.department||'—',ot,hrs,bonus,combined,flag};
  }).filter(r=>r.combined>0).sort((a,b)=>b.combined-a.combined);
  return (
    <div className="p-6">
      <SectionCard title={`Employee Review — ${empData.length} Employees with Activity`} subtitle="Sorted by combined OT + Bonus">
        <DataTable
          ...
          rows={empData.slice(0,200).map(r=>[
            ...
            <Badge key={r.emp} label={r.flag} color={r.flag==='High Priority'?'red':r.flag==='Moderate'?'amber':'emerald'} />
          ])}
        />
      </SectionCard>
    </div>
