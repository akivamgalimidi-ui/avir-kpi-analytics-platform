...
function App() {
  ...
  const refreshData = async () => {
    try {
      ...
      setSystemStatus({ api: 'FAIL', db: 'FAIL', error: err.message });
    }
  };
  ...
  const NavButton = ({ tab }: { tab: typeof TABS[0] }) => (
    <button
      onClick={() => setActiveTab(tab.id)}
      className={`w-full flex items-center gap-3 px-4 py-3 rounded-2xl text-sm font-bold transition-all focus:outline-none focus:ring-2 focus:ring-blue-500 ${
        activeTab === tab.id ? 'bg-blue-600 text-white shadow-xl shadow-blue-900/60' : 'text-slate-400 hover:bg-slate-800 hover:text-slate-200'
      }`}
    >
    ...
    if (loading) {
      return (
        <div className="p-12 flex flex-col items-center justify-center text-center h-[70vh]">
          ...
          <p className="text-slate-500 max-w-sm">Extracting KPIs and normalizing dimensions...</p>
        </div>
      );
    }
        ...
        <div className="p-8 mx-auto max-w-4xl">
          <div className="bg-red-50 border border-red-100 p-8 rounded-3xl flex gap-6 text-red-700 shadow-sm">
            ...
            <div className="flex-1 overflow-hidden">
              ...
              <div className="text-sm font-medium opacity-80 leading-relaxed space-y-2 break-words">
                <p>{error}</p>
              </div>
              <button 
                onClick={() => { setError(null); setUploadResult(null); }} 
      ...
      return (
        <div className="p-8 space-y-8 animate-in fade-in duration-500">
           <div className="grid grid-cols-1 md:grid-cols-4 gap-6">
              {renderDashboardCard('Total Facilities', summary.facilities, MapPin, 'text-blue-600')}
              {renderDashboardCard('Regional Hubs', summary.regions, Globe, 'text-emerald-600')}
        ...
        <header className="h-20 bg-white border-b border-slate-200 px-10 flex items-center justify-between shrink-0 z-10 shadow-sm">
          ...
          <div className="flex gap-4">
             ...
             <label 
                tabIndex={0}
                onKeyDown={(e) => { if (e.key === 'Enter' || e.key === ' ') e.currentTarget.click(); }}
                className={`px-6 py-2.5 rounded-xl text-xs font-black uppercase tracking-widest transition-all shadow-xl flex items-center gap-2 cursor-pointer focus:outline-none focus:ring-4 focus:ring-blue-100 ${
